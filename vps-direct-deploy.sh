...
# BingoMaster Direct VPS Deployment Script
# Copy and paste this entire script into your VPS terminal after SSH login

echo "🚀 Starting BingoMaster deployment on VPS..."
...
apt install -y curl wget git nginx postgresql postgresql-contrib build-essential ufw htop unzip
...
PGPASSWORD=$DB_PASSWORD psql -h localhost -U bingouser -d bingomaster -c "SELECT version();" > /dev/null
if [ $? -eq 0 ]; then
...
# Create comprehensive package.json for BingoMaster
...
cat > package.json << 'PACKAGE_JSON'
{
  ...
  "scripts": {
    ...
    "build:server": "esbuild server/index.ts --bundle --platform=node --outfile=dist/server.js --external:pg-native --format=cjs",
...
mkdir -p {client,server,shared,public/voices,logs,dist}

# Create temporary server for testing
echo "🧪 Creating test server..."
cat > dist/server.js << 'TEST_SERVER'
  ...
  res.send(`
    ...
    <body>
        <div class="container">
            ...
            <h3>Next Steps:</h3>
            <ol>
                <li>Upload BingoMaster source code</li>
                <li>Build the application</li>
                <li>Deploy with PM2</li>
                <li>Configure domain (optional)</li>
            </ol>
            
            <p><strong>Ready for BingoMaster deployment! 🚀</strong></p>
        </div>
    </body>
    </html>
  `);
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    service: 'BingoMaster VPS'
  });
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 BingoMaster test server running on port ${PORT}`);
...
cat > /etc/nginx/sites-available/bingomaster << 'NGINX_CONFIG'
server {
...
cat > /root/backup-bingomaster.sh << 'BACKUP_SCRIPT'
#!/bin/bash
...
echo ""
echo "🔄 Next Steps:"
echo "1. Test the server: visit http://$(hostname -I | awk '{print $1}')"
echo "2. Upload BingoMaster source code to $APP_DIR"
echo "3. Build and deploy the application"
echo ""
echo "✅ Your VPS is ready for BingoMaster deployment!"